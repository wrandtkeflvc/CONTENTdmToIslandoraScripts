...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!--
        Version FLVC-0.9
        Modified to output reference to MODS 3.6, and elided the version="3.x".
...
<xsl:apply-templates/>
    ...
    <xsl:template match="dc:identifier">
        <xsl:if test="starts-with(text(), 'DP')">
            <identifier type="IID">
                    <xsl:value-of select="."/>
            </identifier>
        </xsl:if>
        ...
        <identifier>
            ...
            <xsl:choose>
                ...
                <!-- Skip, we handled this above with type='IID"  -->
                <xsl:when test="starts-with(text(), 'DP')"/>
          		<xsl:otherwise>
          			<xsl:apply-templates/>
          		</xsl:otherwise>
            </xsl:choose>
